...
---
# <a name="nugetconfig-reference"></a>Referencia `nuget.config`

El comportamiento de NuGet se controla mediante la configuración de archivos `NuGet.Config` o `nuget.config` diferentes, como se describe en [Configuraciones comunes de NuGet.](../consume-packages/configuring-nuget-behavior.md)
...
Contiene varios valores de configuración, que se pueden establecer mediante el [ `nuget config` comando](../reference/cli-reference/cli-ref-config.md).
...
| globalPackagesFolder (proyectos que usan solo PackageReference) | La ubicación de la carpeta de paquetes global predeterminada. El valor predeterminado es `%userprofile%\.nuget\packages` (Windows) o `~/.nuget/packages` (Mac o Linux). Se puede usar una ruta de acceso relativa en archivos `nuget.config` específicos del proyecto. Esta configuración se invalida mediante la `NUGET_PACKAGES` variable de entorno , que tiene prioridad. |
| repositoryPath (solo `packages.config`) | La ubicación en la que se van a instalar los paquetes NuGet en lugar de la carpeta `$(Solutiondir)/packages` predeterminada. Se puede usar una ruta de acceso relativa en archivos `nuget.config` específicos del proyecto. Esta configuración se invalida mediante la `NUGET_PACKAGES` variable de entorno , que tiene prioridad. |
...
, , , y funcionan conjuntamente para configurar cómo funciona NuGet con repositorios de paquetes durante las operaciones de `packageSources` `packageSourceCredentials` `apikeys` `activePackageSource` `disabledPackageSources` `trustedSigners` instalación, restauración y actualización.

El [ `nuget sources` comando](../reference/cli-reference/cli-ref-sources.md) se usa generalmente para administrar esta configuración, excepto para la que se administra mediante el comando y que `apikeys` se administra mediante el [ `nuget setapikey` ](../reference/cli-reference/cli-ref-setapikey.md) `trustedSigners` [ `nuget trusted-signers` comando](../reference/cli-reference/cli-ref-trusted-signers.md).
...
| password | La contraseña cifrada para el origen. Las contraseñas cifradas solo se admiten en Windows y solo se pueden descifrar cuando se usan en la misma máquina y a través del mismo usuario que el cifrado original. |
...
Almacena las claves de los orígenes que usan la autenticación de clave de API, tal como se establece con el [ `nuget setapikey` comando](../reference/cli-reference/cli-ref-setapikey.md).
...
Un firmante de confianza tiene una colección de elementos que dan de alta todos `certificate` los certificados que identifican a un firmante determinado. Un firmante de confianza puede ser o `Author` `Repository` .

Un  repositorio de confianza también especifica para el repositorio (que debe ser un URI válido) y, opcionalmente, puede especificar una lista delimitada por punto y coma de para restringir aún más quién es de confianza desde ese repositorio `serviceIndex` `https` `owners` específico.

Los algoritmos hash admitidos que se usan para una huella digital de certificado `SHA256` son y `SHA384` `SHA512` .

Si especifica como certificado dado, se permite encadenar a una raíz que no es de confianza al compilar la cadena de certificados `certificate` como parte de la comprobación de la `allowUntrustedRoot` `true` firma.
...
Si no se encuentra una coincidencia, NuGet comprueba los orígenes de archivo y, a continuación, los orígenes HTTP y, a continuación, descarga los paquetes.
...
| Clave | Value |
| --- | --- |
| format | Valor booleano que indica el formato de administración de paquetes predeterminado. Si `1` es , el formato es PackageReference. Si `0` es , el formato *packages.config*. |
| deshabilitado | Un valor booleano que indica si se debe mostrar el mensaje para seleccionar un formato de paquete predeterminado en la primera instalación del paquete. `False` oculta el símbolo del sistema. |
...
Tenga en cuenta que tiene que usar variables de entorno de estilo Windows (inicia y termina con %) incluso en Mac/Linux. Tener `$HOME/NuGetRepository` en un archivo de configuración no se resolverá. En Mac/Linux, el valor `%HOME%/NuGetRepository` de se resolverá en `/home/myStuff/NuGetRepository` .
...
En la tabla siguiente se muestra la sintaxis de variables de entorno y la compatibilidad con el separador de ruta de acceso para NuGet.Config archivos.

### <a name="nugetconfig-environment-variable-support"></a>`NuGet.Config` compatibilidad con variables de entorno
