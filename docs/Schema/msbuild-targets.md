...
- [Orden de compilación de destinos](#target-build-order)
- [Destino de pack](#pack-target)
- [Escenarios de pack](#pack-scenarios)
- [Destino de restore](#restore-target)
...
Cuando se usa el destino de pack, es decir `msbuild /t:pack`, MSBuild obtiene sus entradas del archivo `.csproj` en lugar de los archivos `project.json` o `.nuspec`. En la tabla siguiente se describen las propiedades de MSBuild que se pueden agregar a un archivo `.csproj` dentro del primer nodo `<PropertyGroup>`. Puede realizar estas modificaciones fácilmente en Visual Studio 2017 y después haciendo clic con el botón derecho en el proyecto y seleccionando **Editar {nombre_proyecto}** en el menú contextual. Para mayor comodidad, la tabla se organiza por la propiedad equivalente en un [archivo `.nuspec`](../schema/nuspec.md).
...
| VersionPrefix | PackageVersionPrefix | vacío | Establecer PackageVersion sobrescribirá PackageVersionPrefix |
