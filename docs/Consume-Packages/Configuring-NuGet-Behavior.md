...
| --- | --- | --- |
| Proyecto | Carpeta del proyecto o cualquier carpeta hasta la raíz de unidad | En una carpeta de proyecto, la configuración se aplica solo a ese proyecto. En las carpetas primarias que contienen varias subcarpetas de proyectos, la configuración se aplica a todos los proyectos de esas subcarpetas. |
| Usuario | Windows: %APPDATA%\NuGet\NuGet.Config<br/>Mac y Linux: ~/.nuget/NuGet.Config | La configuración se aplica a todas las operaciones, pero se reemplaza por la configuración de nivel de proyecto. Cuando se usan comandos de la CLI, se puede especificar un archivo de configuración diferente mediante el modificador `-configFile` para ignorar cualquier configuración en el archivo de nivel de usuario predeterminado. |
| Equipo | Windows: %ProgramFiles(x86)%\NuGet\Config<br/>Mac o Linux: $XDG_DATA_HOME (typically ~/.local/share) | La configuración se aplica a todas las operaciones en el equipo, pero se reemplaza por cualquier configuración de nivel de proyecto o de usuario. |
...
- Para NuGet 2.6 a 3.x, el archivo de configuración de nivel de equipo en Windows se encontraba en %ProgramData%\NuGet\Config[\\{IDE}[\\{Versión}[\\{SKU}]]]\NuGet.Config, donde *{IDE}* podía ser *VisualStudio*, *{Versión}* era la versión de Visual Studio como *14.0* y *{SKU}* era *Community*, *Pro* o *Enterprise*. Para migrar la configuración a NuGet 4.0 y versiones posteriores, simplemente copie el archivo de configuración a %ProgramFiles(x86)%\NuGet\Config. En Linux, esta ubicación anterior era /etc/opt y en Mac, /Biblioteca/Application Support.

## <a name="changing-config-settings"></a>Cambiar los valores de configuración

Un archivo `NuGet.Config` es un archivo de texto XML simple que contiene pares de clave y valor como se describe en el tema [Opciones de configuración de NuGet](../Schema/nuget-config-file.md).

La configuración se administra mediante el [comando config](../tools/cli-ref-config.md) de la CLI de NuGet:
...
Archivo A. Archivo de nivel de usuario, (%APPDATA%\NuGet\NuGet.Config en Windows, ~/.nuget/NuGet.Config en Linux y Mac):
...
>
> Además, ni `NuGetDefaults.Config` ni ningún otro mecanismo de NuGet puede impedir el acceso a orígenes de paquetes como nuget.org. Si una organización quiere bloquear ese acceso, tendrá que usar otros medios (como firewalls) para hacerlo.

### <a name="nugetdefaultsconfig-location"></a>Ubicación de NuGetDefaults.Config

Windows: %ProgramFiles(x86)%\NuGet\Config (NuGet 2.7 a NuGet 3.x: %PROGRAMDATA%\NuGet\NuGetDefaults.Config) Mac o Linux: $XDG_DATA_HOME (normalmente ~/.local/share) 

### <a name="nugetdefaultsconfig-settings"></a>Configuración de NuGetDefaults.Config

- `packageSources`: esta colección tiene el mismo significado que `packageSources` en los archivos de configuración normales y especifica los orígenes predeterminados en su orden preferido. Si esta configuración existe en `NuGetDefaults.Config`, NuGet no usa nuget.org como un origen de paquete predeterminado. Un administrador puede, por tanto, asegurarse de que todos los usuarios que usan este archivo trabajan con los mismos orígenes y evita el uso nuget.org si es necesario.
