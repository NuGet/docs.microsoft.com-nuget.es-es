...
El archivo `project.assets.json` es temporal y no se debe agregar al control de código fuente. Se muestra de forma predeterminada en `.gitignore` y `.tfignore`. Vea [Paquetes y control de código fuente](packages-and-source-control.md).
...
Cuando se usa el formato PackageReference, se puede controlar qué activos de dependencias fluyen al proyecto de nivel superior. Para obtener más información, vea [PackageReference](package-references-in-project-files.md#controlling-dependency-assets).
