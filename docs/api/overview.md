...
Esta API la usa el cliente de Nuget en Visual Studio, NuGet. exe y la CLI de .net para realizar operaciones [`dotnet restore`](/dotnet/core/tools/dotnet-restore?tabs=netcore2x)de NuGet, como buscar en la interfaz de usuario de Visual Studio y. [`nuget.exe push`](../reference/cli-reference/cli-ref-push.md)
...
La versión del esquema de índice de servicio se `version` indica mediante la propiedad en el índice de servicio. La API asigna que la cadena de versión tiene un número de versión principal de `3`. A medida que se realizan cambios no importantes en el esquema de índice de servicio, se aumentará la versión secundaria de la cadena de versión.
...
SUPRIMIR | Elimina o elimina la lista de un recurso.
...
Tiene un único valor para todas las operaciones relacionadas con una única restauración en `PackageReference`. `X-NuGet-Session-Id` En el caso de otros escenarios, como `packages.config` Autocompletar y restaurar, puede haber varios identificadores de sesión diferentes debido a la forma en que se factoriza el código.

## <a name="authentication"></a>Authentication

La autenticación se deja hasta la implementación del origen del paquete que se va a definir. En el caso de Nuget.org `PackagePublish` , solo el recurso requiere autenticación a través de un encabezado de clave de API especial. Vea recursos para obtener más información. [ `PackagePublish` ](package-publish-resource.md)
